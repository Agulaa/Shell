...
public class Shell {
    ...
    private void readCommend() {
        System.out.print(loginService.getLoggedUser().getName()+">");
    ...
    /**
     * Metoda, ktora zostaje wywolana, gdy uzytkownik poda komende 'man'
        ...
        if(command.length>1) {
            ...
            if (command.length == 3) {
                if(ACLController.hasUserPremissionToOperation(fileSystem.getCatalog(), loginService.getLoggedUser(), MODIFY)){ //sprawdzenie uprawnien
                    try {
                        //tworzenie pliku
                        errorFileSystem(fileSystem.createEmptyFile(command[1]));
                        try {
            ...
            if (command.length == 2) {
                if (ACLController.hasUserPremissionToOperation(fileSystem.getFileBase(command[1]), loginService.getLoggedUser(),  MODIFY)) { //sprawdzenie uprawnien
                    try {
                        ...
                        scanner.close();
                        //tworzenie pliku
                       errorFileSystem(fileSystem.createFile(command[1], content));
                        try {
                            ACLController.setDefaultPremissionToFile(fileSystem.getFileBase(command[1]));
                        }catch (Exception e){
                            System.out.println(e.getMessage());
                            readCommend();
                        }
    ...
    /**
     * Metoda, ktora zostaje wywolana gdy uzytkownik poda komende 'cat > [nazwa_pliku] + [ZawartoscDoDodania]'
     * @param command
     */
    private void append(String[] command) {
        if (command.length > 1) {
            //cat > [nazwa_pliku] +zawartość do dodania
    ...
    /**
     * Metoda, ktora zostanie wywolana gdy uzytkwonik poda komende 'cat [nazwa_pliku]'
    ...
    private void cat(String[] command) {
        ...
        if (command.length == 3) {
            if (command[1].equals(">")) {
                if (ACLController.hasUserPremissionToOperation(fileSystem.getFileBase(command[1]), loginService.getLoggedUser(), MODIFY)) {  //sprawdzenie uprawnien
                    append(command); //prywatna metoda, ktora w sobie ma FileSystem.append()
                } else {
                    System.out.println("Brak uprawnien do pliku");
                    readCommend();
                }
        ...
        if (command.length == 2) {
            if (ACLController.hasUserPremissionToOperation(fileSystem.getFileBase(command[1]), loginService.getLoggedUser(), READ)) {  //sprawdzenie uprawnien
                ...
                } else if (fileSystem.readFile(command[1]).charAt(0) == '2') {
    ...
    private void access(String[] command){
        if(command.length==5){
            ...
            if(command[2].equals("--user")){
                ACLController.addAceForUser(command[3],whichMask(command[4]) , fileSystem.getFileBase(command[1]));
            }
            //access [file_name] --group [group_name]
            if(command[2].equals("--group")){
                ACLController.addAceForGroup(command[3],whichMask(command[4]) , fileSystem.getFileBase(command[1]));
            }
        }else{
            System.out.println("Bledna komenda");
            readCommend();
